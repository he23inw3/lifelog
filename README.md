...
* **コンディション & 感情分析**:
  Geminiにより、日記や業務内容から感情（HAPPY 🤩, NORMAL 😐, TIRED 😫, STRESSED 😡 など）を抽出し、ダッシュボード上にグラフで感情の波を可視化します。
...
* **IaC (Terraform) による管理**:
  `infra/terraform/` 以下のファイルで、Cloud Run、Firestore、BigQuery、Cloud Scheduler、Secret Manager、GitHub Actions用の Workload Identity Federation (WIF) 設定までを完全にコード化。Terraform State は GCS でリモート管理しています。
* **CI パイプライン**:
