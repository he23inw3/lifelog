...
/**
 * デモモード無効例外を HTTP 403 Forbidden にマッピングする ExceptionMapper。
 */
@Slf4j
@Provider
public class DemoModeDisabledExceptionMapper implements ExceptionMapper<DemoModeDisabledException> {

    @Override
    public Response toResponse(DemoModeDisabledException exception) {
        log.warn(MessageHelper.getMessage("adapter.provider.demodisabled.log", exception.getMessage()));
