...
class InvalidTokenExceptionMapperTest {
    ...
    private InvalidTokenExceptionMapper target;
    private MockedStatic<TraceHelper> traceHelperMock;

    @BeforeEach
    void setUp() {
        target = new InvalidTokenExceptionMapper();
    ...
    @Test
    @DisplayName("トークン無効例外発生時に HTTP 400 と TraceID を含むエラー応答を返却すること")
    void toResponse_ShouldReturnBadRequestWithErrorResponseAndTraceId() {
        InvalidTokenException exception = new InvalidTokenException("Token has expired");
        ...
        assertThat(body.getMessage()).isEqualTo("Token has expired");
