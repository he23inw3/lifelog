...
class InvalidRequestExceptionMapperTest {
    ...
    private InvalidRequestExceptionMapper target;
    private MockedStatic<TraceHelper> traceHelperMock;

    @BeforeEach
    void setUp() {
        target = new InvalidRequestExceptionMapper();
    ...
    @Test
    @DisplayName("リクエスト検証例外発生時に HTTP 400 と TraceID を含むエラー応答を返却すること")
    void toResponse_ShouldReturnBadRequestWithErrorResponseAndTraceId() {
        InvalidRequestException exception = new InvalidRequestException("Invalid field value");
        ...
        assertThat(body.getMessage()).isEqualTo("Invalid field value");
